...
export default function EditMissing() {
  ...
  const fetchMartyrs = async () => {
    try {
      const res = await getAllMartyrs(100, 1, true);
  ...
  return (
    <div className=" bg-gray-50 p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold text-gray-800">قائمة المفقودين</h1>
        {!loading && (
          <p className="text-sm text-gray-500">{list.length} مفقود</p>
        )}
      </div>
